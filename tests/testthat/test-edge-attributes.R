...
# igraph
  ...
  expect_error(
    edg_get_attr(ig, "fake attr name")
  )
# tidygraph
  expect_identical(
    edg_get_attr(tg, "foo"),
  ...
  expect_error(
    edg_get_attr(tg, "fake attr name")
  )
# network
  expect_identical(
    edg_get_attr(nw, "bar"),
  ...
  expect_error(
    edg_get_attr(nw, "fake attr name")
  )
})

ig_nulls <- igraph::graph_from_edgelist(cbind(edge_attrs$.ego,
                                              edge_attrs$.alter))
ig_empty <- igraph::make_empty_graph()
# network should never have NULL attributes... but it can be forced
# nw_nulls <- network::as.network.matrix(cbind(edge_attrs$.ego,
#                                              edge_attrs$.alter),
#                                        matrix.type = "edgelist")
nw_empty <- network::network.initialize(0)

# empty attributes =====
test_that("empty attributes return NULL", {
# igraph
  expect_null(
    edg_attr_names(ig_nulls)
    )
  expect_null(
    edg_attr_names(ig_empty)
  )
# network
  expect_null(
    edg_attr_names(nw_empty)
  )
})
