...
#' #' library(snatools)
#' #' 
#' #' data("padgett_florence")
#' #' 
#' #' (ig <- as_igraph(padgett_florence))
#' #' 
#' #' (nw <- as_network(padgett_florence))
#' #'
#' 
#' 
#' #' @rdname extract-vertex-attributes 
#' #' 
#' #' @details 
#' #' * `vrt_get_attrs()` extracts all vertex attributes as a `data.frame` or 
#' #' [`tibble::tibble`].
#' #' 
#' #' @examples
#' #' vrt_get_attrs(ig)
...
#' #' @export
#' vrt_get_attrs.igraph <- function(x, out_class = c("tibble", "data.table", "data.frame")) {
#'   out_class <- match.arg(out_class, c("tibble", "data.table", "data.frame"))
#'   out <- as_net_primitive(x)
#'   out <- out[["vertex_attributes"]]
#'   as_df <- choose_df_class(out_class)
#'   if(!is.null(as_df)) {
#'     out <- as_df(out)
#'   }
#'   out
...
#' #' 
#' #' @examples
#' #' vrt_get_attr(ig, vrt_attr = "net_wealth")
#' #' vrt_get_attr(nw, vrt_attr = "priorates")
#' #' 
#' #' @export
#' #' 
