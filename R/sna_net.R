#' The `sna_net` object effectively bridges the `network` objects used by the `statnet` 
...
#' 
#' @details
#' `sna_net` objects contain the following elements: \cr
#' * `graph_attributes`
...
#'        edges.
#'     + `any_loops`: `logical` indicating whether the graph contains loop edges.
#'     + `any_isolates`: `logical` indicating whether the graph contains any isolated 
#'       vertices.
#' * `edges`
...
#'   + If the edge data of an `sna_net` object only include `.ego`s and `.alter`s, `edges`
...
#'      present.
#'      + If an `sna_net` contains a bipartite graph, the second column will always 
...
#' @export
sna_net <- function(edges = NULL, vertices = NULL, is_directed = FALSE) {
  
}
# sna_net <- function(edges = NULL, vertices = NULL, is_directed = TRUE, 
#                     is_bipartite = FALSE, n_actors = NULL, net_attrs = NULL) {
#   if (!is.matrix(edges) && !is.data.frame(edges)) {
#     stop("`edges` must be a `matrix` or `data.frame`.", call. = FALSE)
#   }
#   if (!is.null(vertices) && !is.data.frame(vertices)) {
#     stop("If `vertices` are provided as vertex attributes they must be a `data.frame` with
#           at least one column corresponding to vertex names.", call. = FALSE)
#   }
#   if (!is.logical(is_directed)) {
#     stop("`is_directed` must be either `TRUE` or `FALSE`.", call. = FALSE)
#   }
#   if (!is.logical(is_bipartite)) {
#     stop("`is_bipartite` must be either `TRUE` or `FALSE`.", call. = FALSE)
#   }
#   if(!is_bipartite && !is.null(n_actors)) {
#     stop("`n_actors` is only applicable to bipartite networks and `is_bipartite` you set
#           `is_bipartite` as `FALSE`.", call. = FALSE)
#   }
#   if (!is.null(net_attrs) && is.null(names(net_attrs))) {
#     stop("`n_actors` is only applicable to bipartite networks and `is_bipartite` is 
#          `TRUE`.", call. = FALSE)
#   }
#   if (!is.null(edges)) {
#     if (ncol(edges) < 2L) {
#       stop("`edges` must have 2 columns, one for `.ego`s/from and `.alter`s/to.",
#            call. = FALSE)
#     }
#     if (!is.matrix(edges) && ncol(edges) == 2L) {
#       edges <- as.matrix(edges)
#     }
#     colnames(edges)[[1]] <- ".ego"
#     colnames(edges)[[2]] <- ".alter"
#   }
#   if (!is.null(vertices)) {
#     if ("name" %in% colnames(vertices)) {
#       vertices[[".name"]] <- vertices[["name"]]
#       vertices[["name"]] <- NULL
#     }
#     colnames(vertices[[1]]) <- ".name"
#     if (".actor" %in% colnames(vertices)) {
...
#                    any_loops = vector("logical", 1L),
#                    any_isolates = vector("logical", 1L))
#   
# }
...
#' 
#' @export
as_sna_net <- function(x, ...) {
  UseMethod("as_sna_net")
}

#' @rdname sna_net
#' 
#' @export
as_sna_net.sna_net <- function(x) {
  x
}
...
#' 
#' @export
as_sna_net.network <- function(x, edg_attrs = TRUE, vrt_attrs = TRUE, net_attrs = TRUE) {
  gal <- x[["gal"]]
  # metadata 1
  metadata <- list(n_vertices = as.integer(gal[["n"]]),
                   n_edges = vector("integer", 1L),
  ...
  if (metadata[["n_edges"]]) {
    metadata[["any_loops"]] <- any(el[, 1] == el[, 2])
    metadata[["any_isolates"]] <- !all(seq_len(metadata[["n_vertices"]]) %in% el)
  } else {
    metadata[["any_loops"]] <- FALSE
    metadata[["any_isolates"]] <- FALSE
  }
  ...
  if (metadata[["n_edges"]] && edg_attrs) {
    ...
    if (requireNamespace("data.table", quietly = TRUE)) {
      ...
      edge_attrs <- as.data.frame(edge_attrs, stringsAsFactors = stringsAsFactors,
                                  row.names = NULL)
...
#' @export
as_sna_net.igraph <- function(x, edg_attrs = TRUE, vrt_attrs = TRUE, net_attrs = TRUE) {
  if (net_attrs) {
...
#' @rdname sna_net
#' @export
as_sna_net.tbl_graph <- function(x, edg_attrs = TRUE, vrt_attrs = TRUE, net_attrs = TRUE) {
  as_sna_net(as_igraph(x))
}


#' @export
print.sna_net <- function(x, .nrow = 3L) {
  ...
  descrip <- sprintf("%s %s, %s %s `sna_net`.", 
                     article, dir_chr, multi_chr, bip_chr)
  cat(txt_wrap(descrip, prefix = "# "))
  cat("\n")
  loops_chr <- ifelse(x[["metadata"]][["any_loops"]], ">0", 0L)
  iso_chr <- ifelse(x[["metadata"]][["any_isolates"]], ">0", 0L)
...
#' #' @export
#' print.sna_net <- function(x, n_rows = 2L) {
