...
#' x %>% vrt_get_attrs()
#' 
#' x %>% clarify_actors() %>% vrt_get_attrs()
#' 
#' x %>% swap_actors() %>% vrt_get_attrs()
#' 
#' nw_bipart %>% vrt_get_attrs()
#' 
#' nw_bipart_swap %>% vrt_get_attrs()
#' 
#' clean_bipartite_graph <- function(x, actor_type = TRUE) {
#'   if(!"type" %in% vrt_get_attr_names(x)) {
#'     stop("`x` is not bipartite.")
#'   }
